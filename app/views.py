...
def generer_rapport_stocks_pdf(request, projet_id):
    ...
    try:
        ...
        filename = f"rapport_stocks_{magasin.nom}_{timezone.now().strftime('%Y%m%d_%H%M')}.pdf"
        ...
        def dessiner_en_tete():
            ...
            p.drawImage(logo_path,
                x=largeur/2 - 2*cm, y=y_position - 3*cm,
            ...
            if date_debut_obj and date_fin_obj:
                ...
                p.drawString(2 * cm, y_position - 1.2*cm, f"Période: A partir du {date_debut_obj.strftime('%d/%m/%Y')} jusqu'à aujourd'hui")
            elif date_fin_obj:
                p.drawString(2 * cm, y_position - 1.2*cm, f"Période: Depuis le demarrage du projet jusqu'au {date_fin_obj.strftime('%d/%m/%Y')}")
            else:
                p.drawString(2 * cm, y_position - 1.2*cm, "Période: Depuis le démarrage du projet jusqu'à aujourd'hui.")
            
            p.drawString(2 * cm, y_position - 1.8*cm, f"Généré le: {timezone.now().strftime('%d/%m/%Y à %H:%M')}")
        ...
        for stock_item in stock_items:
            ...
            if y_position < 15 * cm:
                ...
                p.setFont("Helvetica", 10)
                p.drawString(2 * cm, y_position - 1*cm, f"Type d'article: {stock_item.produit.type}")
                y_position -= 2 * cm
            ...
            has_entrees = entrees.exists()
            has_sorties = sorties.exists()
            
            if not has_entrees and not has_sorties:
                # Aucun mouvement - afficher un message
                p.setFont("Helvetica-Oblique", 10)
                p.drawString(2 * cm, y_position, "Aucun mouvement de stocks pour cet article dans cette période.")
                y_position -= 1.5 * cm
                
                # Ajouter quand même la quantité actuelle
                quantite_actuelle = float(stock_item.quantite)
                p.setFont("Helvetica-Bold", 10)
                p.drawString(2 * cm, y_position, f"Quantité actuelle: {quantite_actuelle} {stock_item.produit.unite}")
                y_position -= 1 * cm
                ...
                if has_entrees:
                    ...
                    # Aucune entrée - afficher un message simple
                    p.setFont("Helvetica-Oblique", 9)
                    p.drawString(2 * cm, y_position, "Aucune entrée enregistrée dans cette période.")
                    y_position -= 0.8 * cm
                ...
                if has_sorties:
                    ...
                    for sortie in sorties:
                        data_sorties.append([
                            ...
                            sortie.objet[:28] + "..." if len(sortie.objet) > 30 else sortie.objet,
                    ...
                    # ESPACEMENT UNIFORME entre les articles
                    y_position -= hauteur_sorties + 0.5 * cm
                else:
                    # Aucune sortie - afficher un message simple et la quantité actuelle
                    p.setFont("Helvetica-Oblique", 9)
                    p.drawString(2 * cm, y_position, "Aucune sortie enregistrée dans cette période.")
                    y_position -= 0.8 * cm
